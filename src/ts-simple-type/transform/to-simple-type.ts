		...
		return {
			...
			generic: target => {
				...
				const aliasType: SimpleTypeAlias = {
					...
					fqn: sym ? checker.getFullyQualifiedName(sym) : undefined,
					target,
					typeParameters
				};
...
function toSimpleTypeInternal(type: Type, options: ToSimpleTypeInternalOptions): SimpleType {
	...
	const name = symbol != null ? getRealSymbolName(symbol, ts, options) : undefined;
...
function getRealSymbolName(symbol: ESSymbol, ts: typeof tsModule, options: ToSimpleTypeInternalOptions): string | undefined {
	//const checker = options.checker;
	...
	//const n = checker.getFullyQualifiedName(symbol);
